...
const VaultPage = () => {
  ...
  const fetchVault = async () => {
    try {
      ...
      toast.error('Không thể tải kho CV');
    } finally {
      setLoading(false);
    }
  };
    ...
    if (file) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          toast.error(data.message || 'Lỗi khi tải lên');
        }
      } catch (err: any) {
  ...
  return (
    <div className="max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <h1 className="text-3xl font-black text-on-surface tracking-tight mb-2">Kho lưu trữ CV</h1>
          <p className="text-sm text-on-surface-variant font-medium opacity-70">Xem và quản lý hồ sơ của bạn với định dạng gốc.</p>
        </div>
        <button 
        ...
        >
          ...
          {uploading ? 'Đang xử lý...' : 'Tải lên CV (PDF)'}
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {savedCVs.length > 0 ? (
          savedCVs.map(cv => (
            <div key={cv.id} className="bg-surface-container-low border border-outline-variant/15 rounded-[32px] p-6 group hover:border-primary/40 transition-all duration-300 shadow-sm hover:shadow-xl relative overflow-hidden">
              ...
              <p className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60 mb-8">
                Ngày tải: {new Date(cv.uploadDate).toLocaleDateString()}
              </p>
          ...
          <div className="col-span-full flex flex-col items-center justify-center py-24 bg-surface-container-low border border-dashed border-outline-variant/20 rounded-[40px]">
            ...
            <p className="text-sm font-bold text-on-surface-variant/40 uppercase tracking-widest">Tải lên bản CV thiết kế của bạn</p>
          </div>
        )}
      </div>
            ...
            <div className="px-8 py-6 border-b border-outline-variant/10 flex justify-between items-center bg-surface-container-highest/30">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-primary uppercase tracking-widest">Original PDF Preview</p>
                </div>
              </div>
            ...
            <div className="flex-1 bg-slate-900 overflow-hidden relative">
               {selectedCV.pdfData ? (
                 ...
                 <div className="absolute inset-0 flex flex-col items-center justify-center text-white/50 space-y-4 text-center">
                    ...
                    <p className="text-sm font-bold uppercase tracking-[0.3em]">Bản CV này chưa có dữ liệu PDF gốc</p>
                 </div>
               )}
            </div>
            <div className="p-6 border-t border-outline-variant/10 bg-surface-container-highest/30 flex justify-end">
                <button onClick={() => setSelectedCV(null)} className="bg-on-surface text-surface px-10 py-3.5 rounded-2xl font-black text-xs uppercase tracking-widest hover:opacity-90 transition-opacity">Đóng trình xem</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-surface-container-high w-full max-w-sm rounded-[32px] shadow-2xl border border-outline-variant/15 overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 text-center">
              ...
              <h3 className="text-xl font-bold text-on-surface mb-2">Xác nhận xóa?</h3>
              <p className="text-sm text-on-surface-variant mb-8 leading-relaxed">Bạn có chắc chắn muốn xóa bản CV này khỏi kho lưu trữ? Hành động này không thể hoàn tác.</p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteConfirmId(null)} className="flex-1 bg-surface-container-highest text-on-surface px-6 py-3.5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-outline-variant/20 transition-all">Hủy</button>
                <button onClick={confirmDelete} className="flex-1 bg-error text-on-error px-6 py-3.5 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-error-dim transition-all shadow-lg shadow-error/20">Xóa ngay</button>
              </div>
            </div>
          </div>
        </div>
