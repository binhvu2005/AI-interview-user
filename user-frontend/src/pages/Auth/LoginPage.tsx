...
const LoginPage = () => {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Lỗi khi gửi yêu cầu, vui lòng thử lại');

      toast.success(data.message || 'Mã OTP đã được gửi đến email của bạn');
      setForgotStep(2); // Đi tới bước nhập OTP
    } catch (err: any) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotStep(3); // Đi tới bước nhập mật khẩu mới
    } catch (err: any) {
      toast.error(err.message);
    } finally {
      setLoading(false);
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Lỗi khi đặt lại mật khẩu');

      toast.success('Đặt lại mật khẩu thành công! Vui lòng đăng nhập lại.');
      setForgotStep(0); // Quay lại trang đăng nhập
      setPassword('');
      setOtp('');
      setNewPassword('');
    } catch (err: any) {
  ...
  return (
    <div className="w-full max-w-md bg-surface-container-low rounded-3xl p-6 sm:p-8 shadow-xl border border-outline-variant/15 relative overflow-hidden animate-in zoom-in-95 duration-300">
      ...
      <div className="relative z-10 text-center mb-8">
        <div className="w-14 h-14 mx-auto rounded-2xl bg-surface-container-high border border-outline-variant/20 flex items-center justify-center mb-4 shadow-lg transition-all duration-300">
          <span className="material-symbols-outlined text-primary text-3xl">
            {forgotStep > 0 ? 'mark_email_read' : 'psychiatry'}
          </span>
        </div>
        <h2 className="text-3xl font-black text-on-surface tracking-tight font-headline">
          {forgotStep > 0 ? 'Quên Mật Khẩu' : t('login.title')}
        </h2>
        <p className="text-on-surface-variant text-sm mt-2">
          {forgotStep === 1 ? 'Nhập email của bạn để nhận mã OTP' : 
           forgotStep === 2 ? 'Nhập mã OTP 6 số đã được gửi đến email' :
           forgotStep === 3 ? 'Nhập mật khẩu mới cho tài khoản của bạn' :
           t('login.subtitle')}
        </p>
      </div>
      ...
      {forgotStep === 0 ? (
        ...
        // FORM NHẬP EMAIL ĐỂ LẤY OTP
        <form onSubmit={handleSendOtp} className="relative z-10 space-y-6 animate-in slide-in-from-left-4 duration-300">
          <div>
        ...
        // FORM NHẬP MÃ OTP
        <form onSubmit={handleVerifyOtp} className="relative z-10 space-y-6 animate-in slide-in-from-left-4 duration-300">
          <div>
            <label className="font-headline text-sm font-semibold text-on-surface-variant mb-2 block">Mã OTP (6 chữ số)</label>
            <div className="relative group">
              <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50 text-[20px] group-focus-within:text-primary transition-colors">pin</span>
              <input
                value={otp}
                onChange={(e) => setOtp(e.target.value.replace(/[^0-9]/g, '').slice(0, 6))}
                type="text"
                required
                className="w-full bg-surface-container-lowest border border-outline-variant/15 text-on-surface text-center font-bold text-xl tracking-[0.5em] rounded-xl py-4 focus:border-primary/50 focus:ring-1 focus:ring-primary/20 focus:bg-primary/5 transition-all outline-none placeholder:text-on-surface-variant/30 placeholder:tracking-normal placeholder:font-normal placeholder:text-sm"
        ...
        // FORM NHẬP MẬT KHẨU MỚI (forgotStep === 3)
        <form onSubmit={handleResetPassword} className="relative z-10 space-y-6 animate-in slide-in-from-left-4 duration-300">
          <div>
            <label className="font-headline text-sm font-semibold text-on-surface-variant mb-2 block">Mật khẩu mới</label>
            <div className="relative group">
              <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant/50 text-[20px] group-focus-within:text-primary transition-colors">lock</span>
              <input
                value={newPassword}
